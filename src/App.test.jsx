...
describe("test the handle of fetch Product", () => {
  ...
  test("if the fetch don't work should display a error message", async () => {
    ...
    render(<RouterProvider router={router} />);
    expect(
      await screen.findByRole("heading", { name: "Error no data fetch" })
    ).toBeInTheDocument();
  });
});

describe('test the "Add to card" button in shop-page', () => {
  test("when i click on Atc button should modify the cart logo", async () => {
    const user = userEvent.setup();
    window.fetch = vi.fn(() => {
      const products = [
        {
